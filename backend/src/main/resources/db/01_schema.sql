...
CREATE TABLE IF NOT EXISTS vital_signs (
    id VARCHAR(36) PRIMARY KEY COMMENT '主键ID（UUID）',
    user_id VARCHAR(36) NOT NULL COMMENT '用户ID',
    weight DECIMAL(5,1) COMMENT '今日体重(kg)',
    morning_systolic_pressure INT COMMENT '早上收缩压(mmHg)',
    morning_diastolic_pressure INT COMMENT '早上舒张压(mmHg)',
    evening_systolic_pressure INT COMMENT '晚上收缩压(mmHg)',
    evening_diastolic_pressure INT COMMENT '晚上舒张压(mmHg)',
    blood_sugar DECIMAL(4,1) COMMENT '血糖(mmol/L)',
    water_intake INT COMMENT '今日饮水量(ml)',
